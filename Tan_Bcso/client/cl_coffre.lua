...
-- ===== Menus =====
local evidenceListMenu    = zUI.CreateMenu("Coffres de Saisies", "Sélection", "Actifs", ConfigBcso.themes)
local evidenceActions     = zUI.CreateSubMenu(evidenceListMenu, "Coffre sélectionné", "Actions", "Gestion du coffre", ConfigBcso.themes)
local evidenceConfirmDel  = zUI.CreateSubMenu(evidenceActions, "Confirmer la suppression", "Attention", "Action définitive", ConfigBcso.themes)
local evidenceConfirmForce= zUI.CreateSubMenu(evidenceActions, "Suppression FORCÉE", "Danger", "Perte définitive", ConfigBcso.themes)
...
local function fetchEvidenceList(includeClosed, cb)
    ...
    lib.callback('bcso:evidence:list', false, function(list)
...
local function OpenCreateEvidenceDialog()
    ...
    TriggerServerEvent('bcso:evidence:create', { suspect = suspect, caseId = caseId, notes = notes })
end

RegisterNetEvent('bcso:evidence:created', function(data)
    ESX.ShowNotification(("Coffre créé : %s"):format(data.label or data.id))
    if data and data.id then
        TriggerServerEvent('bcso:evidence:requestOpen', data.id)
...
zUI.SetItems(evidenceActions, function()
    ...
    zUI.Button("🔓 Ouvrir", canOpen and "Accéder à l'inventaire du coffre" or "~c~Coffre scellé", { Disabled = not canOpen }, function(sel)
        ...
        TriggerServerEvent('bcso:evidence:requestOpen', e.id)
        zUI.CloseAll()
    end)
    ...
    zUI.Button(canSeal and "🧷 Sceller" or "✅ Déjà scellé", canSeal and "Interdire toute ouverture" or "~c~Aucune action nécessaire", { Disabled = not canSeal }, function(sel)
        ...
        TriggerServerEvent('bcso:evidence:close', e.id)
...
zUI.SetItems(evidenceConfirmDel, function()
    ...
    zUI.Button("✅ Supprimer", "Irréversible", { RightLabel = "Confirmer" }, function(sel)
        if not sel then return end
        TriggerServerEvent('bcso:evidence:delete', e.id, false)
...
zUI.SetItems(evidenceConfirmForce, function()
    ...
    zUI.Button("✅ Supprimer (FORCÉ)", "~r~Irréversible", { RightLabel = "~r~Confirmer" }, function(sel)
        if not sel then return end
        TriggerServerEvent('bcso:evidence:delete', e.id, true)
...
RegisterNetEvent('bcso:evidence:open', function(stashId)
    exports.ox_inventory:openInventory('stash', stashId)
end)

-- ===== Compat éventuelle : ouvrir un ID reçu d’ailleurs =====
RegisterNetEvent('bcso:evidence:list:select', function(args)
...
exports.ox_target:addBoxZone({
    coords   = ConfigBcso.Coffre.BcsoCoffre.coords,
    size     = ConfigBcso.Coffre.BcsoCoffre.size,
    rotation = ConfigBcso.Coffre.BcsoCoffre.rotation,
    debug    = false,
    options  = {{
        name  = "coffre_bcso_principal",
        icon  = ConfigBcso.Coffre.BcsoCoffre.icon,
        label = ConfigBcso.Coffre.BcsoCoffre.label,
        canInteract = function()
            return hasJob(ConfigBcso.Coffre.BcsoCoffre.jobRequired, 0)
        end,
        onSelect = function()
            exports.ox_inventory:openInventory('stash', ConfigBcso.Coffre.BcsoCoffre.stashId)
        end,
        distance = ConfigBcso.Coffre.BcsoCoffre.distance
    }}
})

-- Coffre Saisies (fixe)
exports.ox_target:addBoxZone({
    coords   = ConfigBcso.Saisies.BcsoSaisies.coords,
    size     = ConfigBcso.Saisies.BcsoSaisies.size,
    rotation = ConfigBcso.Saisies.BcsoSaisies.rotation,
    debug    = false,
    options  = {{
        name  = "coffre_bcso_saisies_fixe",
        icon  = ConfigBcso.Saisies.BcsoSaisies.icon,
        label = ConfigBcso.Saisies.BcsoSaisies.label,
        canInteract = function()
            return hasJob(ConfigBcso.Saisies.BcsoSaisies.jobRequired, 0)
        end,
        onSelect = function()
            exports.ox_inventory:openInventory('stash', ConfigBcso.Saisies.BcsoSaisies.stashId)
        end,
        distance = ConfigBcso.Saisies.BcsoSaisies.distance
    }}
})

-- Salle des preuves (dynamiques)
exports.ox_target:addBoxZone({
    coords   = ConfigBcso.Evidence.zone.coords,
    size     = ConfigBcso.Evidence.zone.size,
    rotation = ConfigBcso.Evidence.zone.rotation,
    debug    = false,
    options  = {
        {
            name  = "evidence_create",
            icon  = ConfigBcso.Evidence.zone.icon,
            label = ConfigBcso.Evidence.zone.labelCreate,
            distance = ConfigBcso.Evidence.zone.distance,
            canInteract = function()
                return hasJob(ConfigBcso.Evidence.job, ConfigBcso.Evidence.minGrade)
            end,
            onSelect = function()
        ...
        },
        {
            ...
            icon  = ConfigBcso.Evidence.zone.icon,
            label = ConfigBcso.Evidence.zone.labelBrowse,
            distance = ConfigBcso.Evidence.zone.distance,
            canInteract = function()
                return hasJob(ConfigBcso.Evidence.job, ConfigBcso.Evidence.minGrade)
            end,
