...
local bcsoStation = ensureVec3(ConfigBcso.BcsoStation) -- Position du commissariat MRPD
...
CreateThread(function()
    while true do
        ...
        local dist = #(pos - bcsoStation) -- OK car bcsoStation est bien un vec3
        ...
        if dist < 10.0 then
            ...
            if dist < 2.0 then
                if not textShown then
                    lib.showTextUI('[E] - 📅 Rendez-vous bcso')
                    textShown = true
                end
...
function OpenRdvMenu()
    ...
    if input then
        TriggerServerEvent("rdv:sendToBcso", input[1], input[2])
        ESX.ShowNotification("✅ Votre demande de rendez-vous a été transmise à la bcso.")
        if textShown then
            lib.hideTextUI()
            textShown = false
        end
